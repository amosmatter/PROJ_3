...
#include "main.h"
#include "stm32u5xx_hal.h" /* Provide the low-level HAL functions */
#include "FATFS/ff.h"
...
#define FCLK_SLOW()                                                                           \
	{                                                                                         \
		MODIFY_REG(SPI.Instance->CFG1, SPI_BAUDRATEPRESCALER_256, SPI_BAUDRATEPRESCALER_128); \
...
void rcv_word(void *x)
{
	for (uint32_t n = 4; n > 0; n--)
		((uint8_t *)x)[n - 1] = xchg_spi(0xFF);
...
)
{
	...
	{ /* Put the card SPI/Idle state */
		LEAVE_INIT_EARLY("Card didn't respond to CMD0 (reset)\n Probably no card inserted!\n");
	}
	...
	if (send_cmd(CMD8, 0x1AA) == 1) /* SDv2? */
	{
		...
		U32 word;
		rcv_word(&word);
		if (word.b[0] != 0xAA || word.b[1] != 0x01)
			/* Does the card support vcc of 2.7-3.6V? */
			LEAVE_INIT_EARLY("The Card doesn't support 2.7-3.6V! \n");

		if (SPI_Timer_Status() && send_cmd(CMD58, 0) != 1)
			LEAVE_INIT_EARLY("Card didn't properly respond to CMD58 (READ OCR)\n");

		rcv_word(&word);

		if ((word.d & 0x00380000) != 0x00380000) /* Does the card support vcc of 3.1-3.4V? */
			LEAVE_INIT_EARLY("Card does not support 3.1-3.4V: DON'T USE THIS CARD!\n");

		while (SPI_Timer_Status() && send_cmd(ACMD41, 1UL << 30))
			; /* Wait for end of initialization with ACMD41(HCS) */

		if (SPI_Timer_Status() && send_cmd(CMD58, 0) != 0)
			LEAVE_INIT_EARLY("Card didn't properly respond to CMD58 (READ OCR) after initialization\n");

		rcv_word(&word);

		if ((word.d & BIT(31)) == 0) /* Check POWER up status */
			LEAVE_INIT_EARLY("Card didn't Successfully power up!\n");

		ty |= (word.d & (BIT(30))) ? CT_BLOCK : 0; /* Check CCS bit in the OCR */
	}
	...
	{ /* Not SDv2 card */
		...
		if (!SPI_Timer_Status() || send_cmd(CMD16, 512) != 0) /* Set block length: 512 */
			LEAVE_INIT_EARLY("Can't set block length\n");
	}
	...
	{ /* Failed */
		LEAVE_INIT_EARLY("Can't find SD card\n");
	}
	DEBUG_PRINT("Card type:");
#ifdef DEBUG
...
)
{
	...
	{
		DEBUG_TRIGGER(); /* Single sector read */

		if (send_cmd(CMD17, sector) != 0) /* READ_SINGLE_BLOCK */
		{
			DEBUG_PRINT("didn't respond to CMD17 (single block request)\n");
			despiselect();
			return RES_ERROR;
		}
		...
		if (!rcvr_datablock(buff, 512))
		{
			...
			DEBUG_PRINT("didn't respond to single block request\n");
			despiselect();
			return RES_ERROR;
		}
